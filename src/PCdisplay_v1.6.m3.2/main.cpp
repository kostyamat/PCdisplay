/*
  ...
  GyverButton              3.5     https://github.com/AlexGyver/GyverLibs
...
#define SENSOR_PIN 14  // датчик температуры    (если 0 то отключено)
...
#if (LED_R && LED_G && LED_B)
int color = 0;
int k, R, G, B, Rf, Gf, Bf, LEDcolor, LEDcolor_val;
...
void brightcontrol()
{
  ...
  {
    photo_val += (analogRead(PHOTO) - photo_val) * 0.1;
#endif
#if (BACKLIGHT && BRIGHT_CONTROL)
    lcd_bright = map((int)photo_val, MIN_PHOTO, MAX_PHOTO, LCD_BRIGHT_MIN, LCD_BRIGHT_MAX);
    ...
    LED = map((int)photo_val, MIN_PHOTO, MAX_PHOTO, LED_BRIGHT_MIN, LED_BRIGHT_MAX);
    LED = constrain(LED, LED_BRIGHT_MIN, LED_BRIGHT_MAX);
#endif
#if (BRIGHT_CONTROL && (LED_GP_PIN || LED_CP_PIN))
    thisBrightness = map((int)photo_val, MIN_PHOTO, MAX_PHOTO, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
...
#if (BACKLIGHT && !BRIGHT_CONTROL)
  lcd_bright = map(PCdata[16], 0, 100, LCD_BRIGHT_MIN, LCD_BRIGHT_MAX);
  lcd_bright = constrain(lcd_bright, LCD_BRIGHT_MIN, LCD_BRIGHT_MAX);
  analogWrite(BACKLIGHT, lcd_bright);
#endif
#if (LED_R && LED_G && LED_B && !BRIGHT_CONTROL)
  ...
  thisBrightness = map(PCdata[16], 0, 100, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
  thisBrightness = constrain(thisBrightness, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
  FastLED.setBrightness(thisBrightness);
#endif
}
...
void updatePlot()
{
  ...
  {
    for (int i = 0; i < 6; i++)
    { // для каждой строки параметров
      for (int j = 0; j < 15; j++)
      {                                    // каждый столбец параметров (кроме последнего)
...
#if (BTN2)
  if (button2.isHolded() && display_mode != 6)
  { // (вторая кнопка)
    display_mode = 6;
    reDraw_flag = 1;
  }
...
void updateDisplay()
{
  if (updateDisplay_flag)
  {
    ...
    case 7:
      ...
      break;
    default:
      debug(); // экран проверки получения данных
    }
...
{
  if (line == 0)
    lcd.printByte(1) else lcd.printByte(4);
  for (int n = 1; n < 9; n++)
  {
    if (n < line)
      lcd.printByte(5);
  ...
  if (line == 10)
    lcd.printByte(6) else lcd.printByte(3);
}
...
#endif
  default:
    mainTemp = max(PCdata[0], PCdata[1]); // взять опорную температуру как максимум CPU, GPU
  }
}
#endif
...
{
  if (PCdata[13] == 1)
  {                        // если стоит галочка Manual Color
    ...
    case 9:
    {
      ...
      if (millis() - prevTime2 > 20)
      {
        ...
        HSVtoRGB(
            constrain(map(fireValue, 20, 60, 0, 10), 0, 255),  // H
            constrain(map(fireValue, 20, 60, 80, 255), 0, 255) // V
        );
      }
    }
    break;
  ...
  else
  {
    if (effLEDTimer.isReady())
    { // если нет
      LEDcolor_val = map(mainTemp, PCdata[11], PCdata[10], 0, 1000);
      LEDcolor_val = constrain(LEDcolor_val, 0, 1000);
      LEDcolor += (LEDcolor_val - LEDcolor) * 0.1; // плавное изменение цвета
...
void HSVtoRGB(uint8_t h, float V)
{
  float H = (float)h / 255;
  int s = int(H * 6);
  float f = H * 6 - s;
...
void timeoutTick()
{
  ...
  if (!restoreConnectToPC && ERROR_UPTIME)
    uptime_timer = millis();
...
void loading()
{
  ...
  if (arcStart >= NUM_LEDS)
    arcStart = (arcStart - NUM_LEDS);
  if (arcEnd >= NUM_LEDS)
    arcEnd = (arcEnd - NUM_LEDS);
...
void show_logo2()
{
  char logo[] = "PCdisplay v1.6.m3.2"; // массив логотипа
  for (int i = 0; i < 9; i++)
  { // показать логотип
    lcd.setCursor(i + 1, 1);
    delay(200);
